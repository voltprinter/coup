...
/* global $, jQuery, alert */
{
  ...
  function openBarcode(master, title, subtitle, expiry, uniq, prefix = 47400) {
    currentCode = master;
    ...
    const couponCode = addCheckDigit(zeroPad(prefix) + zeroPad(uniq) + zeroPad(master, 4));
  }
  ...
  function buildCoupons() {
    const template = $('#coupon-template').html();
  ...
  function getExpiryDate(month) {
    const monthInt = new Date(Date.parse(`${month} 1`)).getMonth();
    const expiryDate = new Date((new Date()).getFullYear(), monthInt + 1, 0);
  ...
  $(document).ready(() => {
    ...
    $('.get-coupon').click(function () {
      const master = $(this).data('mastercode');
